...
class HomeController extends Controller
{
    ...
    public function index() {
        ...
        // Menghitung total pembelian/barang masuk
        $totalPurchase = Purchase::count();

        // Menghitung total penjualan/barang keluar
    ...
    public function currentMonthChart() {
        ...
        $currentMonthSales = Sale::whereIn('status', ['Hutang', 'Lunas'])
            ->whereMonth('date', date('m'))
            ->whereYear('date', date('Y'))
            ->sum('paid_amount') / 100;
        
        $currentMonthPurchases = Purchase::whereIn('status', ['Dicicil', 'Lunas'])
            ->whereMonth('date', date('m'))
            ->whereYear('date', date('Y'))
            ->sum('paid_amount') / 100;
    ...
    //     $currentMonthSales = Sale::where('status', 'Lunas')->whereMonth('date', date('m'))
    //             ->whereYear('date', date('Y'))
    //             ->sum('total_amount') / 100;
    //     $currentMonthPurchases = Purchase::where('status', 'Lunas')->whereMonth('date', date('m'))
    //             ->whereYear('date', date('Y'))
    //             ->sum('total_amount') / 100;
    //     $currentMonthExpenses = Expense::whereMonth('date', date('m'))
    //             ->whereYear('date', date('Y'))
    ...
    public function salesChartData() {
        ...
        $sales = Sale::whereIn('status', ['Hutang', 'Lunas'])
            ->where('date', '>=', $date_range)
            ->groupBy(DB::raw("DATE_FORMAT(date,'%d-%m-%y')"))
            ->orderBy('date')
            ->get([
                DB::raw(DB::raw("DATE_FORMAT(date,'%d-%m-%y') as date")),
                DB::raw('SUM(paid_amount) AS count'),
            ])
            ->pluck('count', 'date');
    ...
    public function purchasesChartData() {
        ...
        $purchases = Purchase::whereIn('status', ['Dicicil', 'Lunas'])
            ->where('date', '>=', $date_range)
            ->groupBy(DB::raw("DATE_FORMAT(date,'%d-%m-%y')"))
            ->orderBy('date')
            ->get([
